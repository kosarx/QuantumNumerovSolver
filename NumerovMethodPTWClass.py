...
class NumerovSolverPTW(NumerovSolverPIB):
    ...
    def set_phys_parameters(self, phys_attrib_list):
        ...
        self.well_width = phys_attrib_list[2]
        del self.L
    ...
    def set_potential(self, x, Potential_function):
        ...
        self.Potential = self.Potential_function(x, self.well_width, self.V0)
    ...
    def reset_parameters(self, phys_attrib_list, Energy, Potential_function, npoints=1000):
        ...
        self.x_max = 3 if self.quant_num<=3 else 2+np.sqrt(1.6*self.quant_num+1)-1/(self.quant_num+1) # scale x range as quant_num expands
    ...
    def plot_show(self, matching=False):
        ...
        plt.plot(self.x, self.Potential, color = 'g', linestyle='-', label=r'$V_0/E$', linewidth=2)

        plt.fill_between(self.x, -self.V0-2, self.Potential, color='gray', alpha=0.3)
